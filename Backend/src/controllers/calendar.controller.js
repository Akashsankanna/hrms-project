"use strict"
const { sendMeetingInvite } = require("../services/mail.service")
...
// recipients = [{ workEmail, firstName, lastName }]

function sendEventEmails(recipients, event, visibility, departmentName) {
  ...
  Promise.allSettled(
    ...
    console.log(
      `[EMAIL] ${recipients.length} invite(s) sent for "${event.title}" [${visibility}]`
    )
  )
}
...
async function getHolidays(req, res) {
  try {
    const {
      year = new Date().getFullYear(), month, type, stateCode,
    } = req.query
  ...
  try {
    ...
    if (myEvents === "true" && employeeId) {
      ...
      visibilityFilter = [
        ...
        { attendees: { some: { employeeId } } },
        ...(myDepartmentId
          ? [{ visibility: "DEPARTMENT", departmentId: myDepartmentId }]
          : []),
      ]
    } else {
...
// ── CREATE EVENT — emails sent based on visibility ────────────────────────────
...
async function createEvent(req, res) {
  try {
    ...
    if (visibility === "ASSIGN" && attendeeIds.length === 0)
      return res.status(400).json({ error: "attendeeIds required for ASSIGN visibility" })
    ...
    if (!targetEmployeeIds.includes(createdById))
      targetEmployeeIds.push(createdById)
    ...
    const event = await prisma.$transaction(async (tx) => {
      ...
      // In-app notifications (everyone except creator)
    ...
    // ── Send emails based on visibility ───────────────────────
    // Skip creator (they created it, no need to notify themselves)

    const emailRecipients = fullEvent.attendees
      .filter(a => a.employeeId !== createdById && a.employee?.workEmail)
      .map(a => ({
        workEmail: a.employee.workEmail,
    ...
    return res.status(201).json({
      success: true,
      event:   fullEvent,
      invited: targetEmployeeIds.length,
    })
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (addAttendeeIds.length > 0) {
        const existingA   = await tx.eventAttendee.findMany({
...
async function inviteAttendees(req, res) {
  try {
    ...
    const newOnes     = [...new Set(employeeIds.map(Number))].filter(
      eid => !existingIds.has(eid)
    )
    if (newOnes.length === 0)
      return res.json({ success: true, message: "All already invited", added: 0 })
...
async function getMyCalendar(req, res) {
  try {
    const {
      from = new Date().toISOString().split("T")[0], to,
    } = req.query
    ...
    const [events, holidays, attendanceSummaries] = await Promise.all([
      ...
      prisma.attendanceSummary.findMany({
        where:   { employeeId, attendanceDate: { gte: fromDate, lte: endDate } },
...
async function getTeamView(req, res) {
  try {
    const {
      from = new Date().toISOString().split("T")[0], to, departmentId,
    } = req.query
    const toDate    = to || from
    const fromDate  = new Date(from)
    const endDate   = new Date(toDate + "T23:59:59")
        ...
        include: {
          event: {
            select: {
              ...
              endTime: true, eventType: true, meetLink: true,
            },
          },
        },
      }),
    ...
    eventAttendances.forEach(ea => {
      if (!eventAttendanceMap.has(ea.employeeId))
        eventAttendanceMap.set(ea.employeeId, [])
...
async function getAdminOverview(req, res) {
  try {
    ...
    const [
      totalEvents, totalHolidays,
      upcomingEvents, recentEvents, typeBreakdown,
    ] = await Promise.all([
